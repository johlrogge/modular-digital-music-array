...
// Separate benchmark specifically for time-to-playable
    ...
    for name in ["short.flac", "medium.flac", "long.flac"] {
        group.bench_with_input(BenchmarkId::from_parameter(name), name, |b, name| {
            ...
            b.iter_with_large_drop(|| {
                ...
                track.play(); // Prepare for playback
                let ready_time = start.elapsed();

                println!("\nTime to playable for {}:", name);
                println!("  Total time: {:?}", ready_time);
...
fn bench_track_loading(c: &mut Criterion) {
    ...
    for name in ["short.flac", "medium.flac", "long.flac"] {
        group.bench_with_input(BenchmarkId::from_parameter(name), name, |b, name| {
            let path = test_file_path(name);
            b.iter_with_large_drop(|| {
                let (track, metrics) = Track::new(black_box(&path)).expect("Failed to load track");
                println!("\nMetrics for {}:", name);
                ...
                println!(
                    ...
                    metrics.decoding_stats.packet_read_time.as_secs_f64() * 100.0
                        / metrics.decoding_time.as_secs_f64()
                );
                println!(
                    "  Packet decoding:    {:?} ({:.1}%)",
                    metrics.decoding_stats.packet_decode_time,
                    metrics.decoding_stats.packet_decode_time.as_secs_f64() * 100.0
                        / metrics.decoding_time.as_secs_f64()
                );
                println!(
                    "  Sample copying:     {:?} ({:.1}%)",
                    metrics.decoding_stats.sample_copy_time,
                    metrics.decoding_stats.sample_copy_time.as_secs_f64() * 100.0
                        / metrics.decoding_time.as_secs_f64()
                );
                ...
                println!(
                    ...
                    metrics.decoding_stats.total_packet_bytes
                        / metrics.decoding_stats.packets_processed
                );
                println!(
