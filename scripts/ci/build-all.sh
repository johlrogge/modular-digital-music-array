...
#
# Uses `cargo polylith cargo --profile <profile> <cmd>` which generates the
# profile workspace manifest and runs the cargo command in one step.
# dev profile is used for all binaries except mdma-acid.
# production profile is used for mdma-acid (file-backed fact-store).
#
