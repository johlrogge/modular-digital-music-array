...
## Pattern: Borrowed vs Owned

### Bad: Mixed Ownership
```rust
// ❌ Unclear if borrowed or owned
fn process(data: &str) {
    // Can't tell if data will be stored
}
```
...
### Bad: Global Permissions
```rust
// ❌ Anyone can access filesystem
fn read_file(path: &str) -> String {
    std::fs::read_to_string(path).unwrap()
}
```

### Good: Capability Token
```rust
// ✅ Must have capability to read
struct FileSystem {
    root: PathBuf,
}
...
// Pass FileSystem capability where needed
fn process(fs: &FileSystem) {
    let data = fs.read("config.toml").unwrap();
}
```

## Pattern: Positive vs Negative Information

### Bad: What's Missing
```rust
// ❌ Tracks what's NOT done
struct Task {
    completed_steps: Vec<Step>,
    // How do we know which steps remain?
}
```

### Good: What Remains
```rust
// ✅ Explicitly tracks remaining work
struct Task {
