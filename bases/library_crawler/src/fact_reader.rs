...
impl FactAggregator<ContentHash, MusicValue, FactSource> for AggregatedTrack {
    fn assert(&mut self, value: &MusicValue, _source: &FactSource) {
        ...
        match value {
            ...
            Source(s) => self.source = Some(s.clone()),
            ItemId(s) => self.item_id = Some(s.clone()),
            ...
            TrackStarted(_) | TrackStopped(_) => {} // play history — not aggregated metadata
        }
    }
    ...
    fn retract(&mut self, value: &MusicValue, _source: &FactSource) {
        ...
        match value {
            ...
            Source(_) => self.source = None,
            ItemId(_) => self.item_id = None,
            ...
            TrackStarted(_) | TrackStopped(_) => {} // play history — not retractable metadata
        }
    }
